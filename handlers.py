...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): user = update.effective_user logger.info(f"📥 /start from {user.username or user.id}") await update.message.reply_text("👋 Hello! I'm your Homework Forwarder Bot. Drop homework, and I’ll pass it along!")
# /id command: show the chat ID
async def chat_id(update: Update, context: ContextTypes.DEFAULT_TYPE): chat = update.effective_chat logger.info(f"📥 /id command from {update.effective_user.username or update.effective_user.id}") await update.message.reply_text(f"🆔 Chat ID: {chat.id}", parse_mode='Markdown')
# /status command: check bot health
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE): user = update.effective_user logger.info(f"📥 /status from {user.username or user.id}")
status_msg = (
    "✅ *Bot Status*\n"
...
async def reload_config(update: Update, context: ContextTypes.DEFAULT_TYPE): user = update.effective_user admin_id = context.bot_data.get("ADMIN_CHAT_ID")
if user.id != admin_id:
...
async def forward_message(update: Update, context: ContextTypes.DEFAULT_TYPE): try: message = update.message if not message: logger.warning("⚠️ No message found in the update.") return
...
# Safe MarkdownV2 escaping for admin message
    escaped_sender = escape_markdown(sender_name, version=2)
    escaped_media_type = escape_markdown(media_type, version=2)
    escaped_source_id = escape_markdown(str(source_id), version=2)
    admin_text = f"📫 *Forwarded {escaped_media_type}* from {escaped_sender} \chat ID: {escaped_source_id}\"
